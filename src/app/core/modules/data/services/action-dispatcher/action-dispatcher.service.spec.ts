...
describe('Service: ActionDispatcher', () => {
  ...
  beforeEach(() => {
    ...
    operationsTracker.getOperationStatus = jasmine.createSpy('getOperationStatus').and.returnValue(of({}));
  ...
  it('should call "getOperationStatus" with required params', async () => {
    ...
    // Assert
    expect(operationsTracker.getOperationStatus).toHaveBeenCalledWith(fakeOperationId, fakePartition);
  });
  ...
  it('should throw an error from operation tracker when the error was tracked', async () => {
    ...
    operationsTracker.getOperationStatus = jasmine
