...
export class BaseDataFilterComponent implements OnChanges, OnDestroy {
  ...
  private fillMappingsInDefs(): void {
    if (this.data && this.data.length) {
      const defsWithoutMapping = this.builtFilteringDefinition.filter((t) => !t.options);
      if (defsWithoutMapping.length) {
        defsWithoutMapping.forEach((item) => {
          const mapped = this.data.map((t) => item.propertySelector(t)).map((t) => (Array.isArray(t) ? [...t] : [t]));

          const distinctValues = distinct(mapped.reduce((a, b) => a.concat(b), []));
          ...
          item.options = sortedValues.map((v) => {
            const option: FilterOption<any> = { value: v };
  ...
  private buildFormGroup(): void {
    ...
    this.builtFilteringDefinition.forEach((def) => {
      ...
      def.options.forEach((opt) => {
        innerFormGroupConfig[opt.optionId] = new FormControl(false);
      });
